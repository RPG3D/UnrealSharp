...
bool FCSDotNetRuntimeHost::EnsureRuntimeDllsExtracted(FString& OutRuntimeDir, TArray<FString>& OutSourceDllNames)
{
	...
	// BCL managed DLLs — staged NonUFS (outside PAK) by CoreClrSDK.Build.cs at
	// Source/ThirdParty/CoreClrSDK/Android/runtime/. Readable via FFileHelper
	// (UE I/O resolves NonUFS files through the Android platform file).
	const FString BclSourceDir = FPaths::Combine(
		UnrealSharp::Paths::GetPluginDirectory(),
		TEXT("Source/ThirdParty/CoreClrSDK/Android/runtime"));

	// Project managed DLLs + LoadOrder.json — staged UFS (inside PAK) at
	// Content/Managed/Android/. Hot-updatable.
	const FString ProjectSourceDir = FPaths::Combine(
		FPaths::ProjectContentDir(), TEXT("Managed/Android"));

	// Collect file names from the PAK/NonUFS sources (not the extraction dir) so the
	// TPA reflects what's currently in the PAK — a DLL removed from the PAK won't be
	// loaded even if it lingers in the extraction dir.
	TArray<FString> BclNames;
	TArray<FString> ProjectNames;
	ExtractDirToRuntime(BclSourceDir, OutRuntimeDir, {TEXT(".dll")}, BclNames);
	ExtractDirToRuntime(ProjectSourceDir, OutRuntimeDir, {TEXT(".dll"), TEXT(".pdb"), TEXT(".json")}, ProjectNames);

	// TPA only wants .dll names (BCL + project). Sidecar .xxhash / .pdb / .json are
	// excluded — filter the project names to .dll only.
	OutSourceDllNames = BclNames;
...
bool FCSDotNetRuntimeHost::InitializeManagedRuntimeAndroid()
{
	...
	{
		UE_LOGFMT(LogUnrealSharp, Fatal, "Failed to prepare Android CoreCLR runtime directory.");
		return false;
	}
	...
	UE_LOG(LogUnrealSharp, Log, TEXT("[CoreClr-Android] coreclr_initialize..."));
	...
	UE_LOG(LogUnrealSharp, Log, TEXT("[CoreClr-Android] coreclr_initialize => 0x%x (domain %u)"),
	...
	UE_LOG(LogUnrealSharp, Log, TEXT("[CoreClr-Android] coreclr_create_delegate => 0x%x, fn=%p"),
	...
	UE_LOG(LogUnrealSharp, Log, TEXT("[CoreClr-Android] UnrealSharp initialized successfully."));
	return true;
}
