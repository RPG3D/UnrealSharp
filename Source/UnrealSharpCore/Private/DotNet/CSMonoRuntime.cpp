...
MonoDomain* InitializeMonoRuntime(const FString& RuntimeDir, const FString& ExtraSearchPaths)
{
	...
	// "debug" surfaces Mono's internal GC/threadpool trace for diagnostics;
	// Test/Shipping builds drop to "warning" to avoid log spam.
...
#if PLATFORM_ANDROID
	// Step 0.5: Redirect stderr to a file so Mono's crash handler output
	// (managed thread dump) is capturable — Android stderr defaults to /dev/null.
	// ProjectDir() is a virtual relative path on Android (UFS abstraction);
	// ProjectPersistentDownloadDir() returns a real absolute path (/data/data/<pkg>/files/).
	const FString StderrLogPath = FPaths::Combine(FPaths::ProjectPersistentDownloadDir(), TEXT("mono_stderr.log"));
	const int StderrFd = open(TCHAR_TO_UTF8(*StderrLogPath), O_WRONLY | O_CREAT | O_TRUNC, 0644);
	if (StderrFd >= 0)
	{
		dup2(StderrFd, STDERR_FILENO);
		dup2(StderrFd, STDOUT_FILENO); // mono's g_async_safe_printf writes to fd 1 (stdout)
		close(StderrFd);
		UE_LOG(LogUnrealSharp, Log, TEXT("[Mono] stderr/stdout redirected to %s"), *StderrLogPath);
	}
	else
	{
		UE_LOG(LogUnrealSharp, Error, TEXT("[Mono] Failed to open stderr log (%s): %hs"), *StderrLogPath, strerror(errno));
	}
#endif
...
#if PLATFORM_IOS
	// iOS: AOT mode (INTERP) is MANDATORY - iOS bans JIT (W^X kernel enforcement),
	// so it must be set regardless of debugger state. InitMonoDebugger only calls
	// mono_jit_parse_options for the debugger-agent; it does NOT set AOT mode.
	// (The old !GMonoDebuggerActive guard skipped this on debugger, leaving iOS in
	// default JIT -> W^X crash, and CoreLib AOT/DL fallback/globalization unregistered.)
	{
...
#else
	// Non-iOS: when the debugger is active, InitMonoDebugger already configured
	// JIT via mono_jit_parse_options - skip to avoid overriding it.
	...
	// Step 4: Signal chaining — let Mono's SIGSEGV handler forward to the OS
	// so native crashes produce Android tombstones instead of silent process death.
	mono_set_signal_chaining(1);
	// Crash chaining: print Mono's crash report (native + managed thread dump,
	// via stderr which is redirected to mono_stderr.log) BEFORE chaining onward.
	// Without this, Mono chains the signal to the previous handler immediately
	// and skips its crash report entirely.
	mono_set_crash_chaining(1);
	UE_LOG(LogUnrealSharp, Log, TEXT("[Mono] Signal chaining enabled (native crashes will produce tombstones)"));

	// Step 5: Initialize JIT
...
void ShutdownMonoRuntime(MonoDomain* Domain)
{
	if (Domain)
	{
		...
		// Signal the runtime that we're shutting down so internal threads
		// (GC finalizer, threadpool) stop before mono_jit_cleanup frees
		// their mutexes. Without this, FORTIFY crashes on destroyed mutexes.
